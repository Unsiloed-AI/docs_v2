...
openapi: "/api-reference/parser/openapi-v1.json POST /parse"
...
<Tip>
  Processing large files or running many requests in parallel? The [Presigned Upload endpoint](/api-reference/parser/parse-document-v2) (`POST /v2/parse/upload`) supports faster uploads, larger file sizes, and higher throughput.
</Tip>
...
<ParamField body="ocr_engine" type="string">
  ...
  - `"UnsiloedHawk"` (default): Higher accuracy for complex layouts and mixed content.
...
```text 402 - Insufficient Quota
Insufficient quota — not enough page credits remaining.
```
