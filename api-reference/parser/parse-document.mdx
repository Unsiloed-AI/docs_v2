...
<ParamField body="validate_segments" type="string">
  JSON array of segment types to validate using VLM (Vision Language Model). When specified, the listed segment types are validated and corrected for misclassifications. Pass as a JSON-encoded string array. Example: `'["Table","Picture","Formula"]'`. Supported segment types: `Table`, `Picture`, `Formula`.
</ParamField>
...
10. **Usage Limit Exceeded** (`429`): Billing usage cap reached. Returns plain text: `Usage limit exceeded`. No `Retry-After` header.
11. **Rate Limit Exceeded** (`429`): Org exceeded 60 requests / 60s sliding window. Returns plain text: `Rate limit exceeded` with `Retry-After: 1` header.