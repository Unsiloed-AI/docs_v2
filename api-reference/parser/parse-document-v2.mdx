...
<ParamField body="extract_charts" type="boolean">
  Extract structured data from charts and graphs using the vision model. Defaults to `false`.
</ParamField>
...
<ParamField body="validate_segments" type="array">
  Segment types to validate with VLM. Example: `["Table", "Formula"]`.
</ParamField>
...
POST to `/v2/parse/upload` with your file name and configuration. The API returns a `job_id` and a short-lived presigned S3 URL.
...
```json
{
  ...
  "upload_url": "https://unsiloed-bucket.s3.us-east-1.amazonaws.com/uploads/a3f1c2d4.../report.pdf?X-Amz-Algorithm=AWS4-HMAC-SHA256&...",
...
Use the `upload_url` and `upload_headers` from the response to PUT your file straight to S3. No API key is needed for this request.
...
<Warning>
  You must include every header listed in `upload_headers`. A missing or mismatched `Content-Type` will cause S3 to reject the upload with a `403`.
</Warning>
